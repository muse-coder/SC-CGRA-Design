...
def compute_mse(X, Y):
    """ compute mean square error of two images Parameters: ----------- X, Y: numpy array two images data Returns: -------- mse: float mean square error """
    X = np.float32(X)
    Y = np.float32(Y)
    mse = np.mean((X - Y) ** 2, dtype=np.float64)
    return mse
def compute_psnr(X, Y, data_range):
    """ compute peak signal to noise ratio of two images Parameters: ----------- X, Y: numpy array two images data Returns: -------- psnr: float peak signal to noise ratio """
    mse = compute_mse(X, Y)
...
def color_to_gray(image, normalization=False):
    """ convert color image to gray image Parameters: ----------- image: numpy array color image data normalization: bool whether to do nomalization Returns: -------- image: numpy array gray image data """
...
def resample(image, factor, interpolation):
    """ down sample and then upsample image by factor using a certain interpolation method Parameters: ----------- image: numpy array image data factor: sampling factor interpolation: enum interpolation type Returns: -------- image_resample: numpy array resampled image data """
...
if __name__ == '__main__':
    ...
    image_original = cv2.imread('./sharpen_approximate_.png',cv2.IMREAD_GRAYSCALE)
    image_resample =  cv2.imread('./sharpen_accurate_.png',cv2.IMREAD_GRAYSCALE)
    # psnr
